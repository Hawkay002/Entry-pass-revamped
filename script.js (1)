...
import { getFirestore, collection, doc, setDoc, addDoc, onSnapshot, getDocs, getDoc, query, deleteDoc, updateDoc, where, orderBy, limit, writeBatch } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
...
function resetAppState() {
    ...
    if (typeof chatUnsubscribe === 'function') chatUnsubscribe(); // Cleanup chat listener
...
function normalizeRecord(raw) {
    ...
    return { 
    id: getVal(['ticketid', 'id', 'ticket id']),
    ...
    status: statusVal, 
    entryTimeRaw: getVal(['entrytime', 'entry', 'scannedat', 'time', 'entry time']) 
    };
}
// -------------------------------------------------------------
    ...
    try {
        for (const record of parsedImportData) {
            if (existingPhones.has(record.phone)) {
                ...
                // NEW: Define the ticket data object first
                const ticketData = {
                    name: record.name,
...
// NEW: Check if ID exists to determine method 
                if (record.id) {
// Use setDoc to restore the ORIGINAL ID
                    const originalId = String(record.id).trim();
                    await setDoc(doc(db, APP_COLLECTION_ROOT, SHARED_DATA_ID, 'tickets', originalId), ticketData);
                } else {
// Fallback to addDoc (generates random ID) if no ID found
                    await addDoc(collection(db, APP_COLLECTION_ROOT, SHARED_DATA_ID, 'tickets'), ticketData);
                }
...
const clearNotifsBtn = document.getElementById('clearNotifsBtn'); // NEW
...
// NEW: Reply & Context Menu Elements
...
// NEW: Selection Menu Elements
const chatSelectionMenu = document.getElementById('chatSelectionMenu');
...
// State
let isChatSelectionMode = false; // NEW
let selectedMessageIds = new Set(); // NEW

// State
let isChatOpen = false;
let isNotifOpen = false;
let totalUnreadCount = 0;
let previousTotalUnread = 0; // Track previous count to prevent loops
let chatUnsubscribe = null;
const notificationAudio = new Audio('/chime.mp3'); 

// Data Storage
let allMessages = []; 
let lastMessageByChannel = {}; 
let unreadCounts = {}; 

// Active Chat State
let activeChatChannel = null; // { type, target } or null (for list view)
let replyingTo = null; 
...
function toggleChat(forceOpen = false) {
    if (forceOpen || !isChatOpen) {
        ...
        isChatOpen = true;
        if (isNotifOpen) toggleNotifications();
        
        // Default to list view on open unless actively redirected
        if (!activeChatChannel) showChannelList();
    } else {
        chatDrawer.classList.remove('open');
        isChatOpen = false;
        if (typingUnsubscribe) typingUnsubscribe(); // Stop listening
        cancelReply();
        exitSelectionMode(); // NEW: Clear selection when closing
    }
}
...
function toggleNotifications() {
    if (!isNotifOpen) {
        ...
        renderNotifications(); // Populate list
    } else {
...
if (clearNotifsBtn) {
    clearNotifsBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        // Save current time. Any notification older than this won't show.
        localStorage.setItem('notifs_cleared_timestamp', Date.now());
        renderNotifications(); 
        showToast("Notifications Cleared", "List has been emptied.");
    });
}
...
window.addEventListener('click', (e) => {
    if (isNotifOpen && !notificationDropdown.contains(e.target) && !notificationBtn.contains(e.target)) {
        toggleNotifications();
    }
    if (chatContextMenu.style.display === 'flex') {
        chatContextMenu.style.display = 'none';
    }
});

// ------------------------------------
// 2. REALTIME LISTENER & DATA PROCESSING
// ------------------------------------

function setupChatListener() {
    if (chatUnsubscribe) chatUnsubscribe();

    // Fetch last 100 messages for efficiency
    const q = query(collection(db, 'communications'), orderBy('timestamp', 'desc'), limit(300));

    chatUnsubscribe = onSnapshot(q, (snapshot) => {
        allMessages = [];
        snapshot.forEach(doc => allMessages.push({ id: doc.id, ...doc.data(), hasPendingWrites: doc.metadata.hasPendingWrites }));
        allMessages.reverse(); // Sort oldest to newest for chat flow
...
function processAllMessages() {
    ...
    // Process messages to build channel data
    allMessages.forEach(msg => {
        // Determine Channel Key based on message type
        let channelKey = getChannelKey(msg);
        
        // Filter: Is this message relevant to me?
        if (!isMessageRelevant(msg)) return;

        // Save Last Message
        lastMessageByChannel[channelKey] = msg;

        // --- FIX START: Intelligent Unread Counting ---
        // 1. Get the time we last read this channel (default to 0 if never opened)
        const lastReadTime = parseInt(localStorage.getItem(`lastRead_${channelKey}`)) || 0;

        // 2. Only count if the message is NEWER than when we last looked
        if (msg.senderEmail !== currentUser.email && !msg.hasPendingWrites && msg.timestamp > lastReadTime) {
             
             if (!unreadCounts[channelKey]) unreadCounts[channelKey] = 0;
             
             // 3. If this channel is CURRENTLY open, don't count it, AND update the read time
             if (activeChatChannel && getActiveChannelKey() === channelKey) {
                 localStorage.setItem(`lastRead_${channelKey}`, Date.now());
             } 
             else {
                 // Channel is closed, so this is a legitimate unread message
                 unreadCounts[channelKey]++;
             }
        }
        // --- FIX END ---
    });
    ...
    // Sum total unread
    Object.values(unreadCounts).forEach(count => totalUnreadCount += count);
    updateBadge();
    ...
    if (totalUnreadCount > previousTotalUnread && !isChatOpen) {
        notificationAudio.play().catch(()=>{}); 
    }
    
    // Update tracker for next run
    previousTotalUnread = totalUnreadCount;
}

function isMessageRelevant(msg) {
    if (msg.channelType === 'GLOBAL') return true;
    if (msg.channelType === 'TEAM') {
        return currentUser && (msg.target === currentUser.email || msg.senderEmail === currentUser.email);
    }
    if (msg.channelType === 'PRIVATE') {
        // I am target OR I am sender
        return currentUsername && (msg.target === currentUsername || msg.senderDisplay === currentUsername);
    }
    return false;
}

function getChannelKey(msg) {
    if (msg.channelType === 'GLOBAL') return 'GLOBAL_ALL';
    if (msg.channelType === 'TEAM') return `TEAM_${msg.target}`; // target is email
    if (msg.channelType === 'PRIVATE') {
        // Key should be consistent regardless of sender/receiver
        // Ideally sorting usernames: PRIVATE_userA_userB
        // For Admin List View, we want to group by the OTHER person
        const otherPerson = msg.senderDisplay === currentUsername ? msg.target : msg.senderDisplay;
        return `PRIVATE_${otherPerson}`; 
    }
    return 'UNKNOWN';
}
...
function getActiveChannelKey() {
    ...
    if (activeChatChannel.type === 'GLOBAL') return 'GLOBAL_ALL';
    if (activeChatChannel.type === 'TEAM') return `TEAM_${activeChatChannel.target}`;
    if (activeChatChannel.type === 'PRIVATE') return `PRIVATE_${activeChatChannel.target}`;
    return null;
}
...
function showChannelList() {
    ...
    chatHeaderTitle.textContent = "Comms Center";
    channelListContainer.style.display = 'flex';
...
async function renderChannelList() {
    ...
    // 1. Broadcast Channel
    const globalMsg = lastMessageByChannel['GLOBAL_ALL'];
    renderChannelItem('Global Broadcast', 'GLOBAL', 'ALL', 'fa-bullhorn', globalMsg);

    // 2. Team Channels (If applicable)
    // For Admin: Show all known teams. For Staff: Show only their team.
    const teams = [
    ...
    if (currentUser.email !== ADMIN_EMAIL) {
        visibleTeams = teams.filter(t => t.email === currentUser.email);
    }

    if (visibleTeams.length > 0) {
        const teamHeader = document.createElement('div');
        teamHeader.className = 'channel-group-header';
        teamHeader.textContent = 'Team Channels';
        channelList.appendChild(teamHeader);

        visibleTeams.forEach(t => {
            const key = `TEAM_${t.email}`;
            renderChannelItem(t.name, 'TEAM', t.email, 'fa-users', lastMessageByChannel[key]);
        });
    }

    // 3. Individual Channels
    const privateHeader = document.createElement('div');
    privateHeader.className = 'channel-group-header';
    privateHeader.textContent = 'Individual Channels';
    channelList.appendChild(privateHeader);

    // ADMIN: Show all available users (Directory)
    if (currentUser.email === ADMIN_EMAIL) {
        // Fetch all usernames once
        try {
            const q = query(collection(db, 'allowed_usernames'));
            const snap = await getDocs(q);
            
            if (snap.empty) {
                channelList.innerHTML += '<div class="notif-empty">No users found</div>';
            } else {
                snap.forEach(doc => {
                    const username = doc.id;
                    const key = `PRIVATE_${username}`;
                    const msg = lastMessageByChannel[key];
                    renderChannelItem(username, 'PRIVATE', username, 'fa-user', msg);
                });
            }
        } catch (e) {
        ...
        // STAFF: Only show active chats (where history exists)
        const privateKeys = Object.keys(lastMessageByChannel).filter(k => k.startsWith('PRIVATE_'));
        
        if (privateKeys.length === 0) {
            const empty = document.createElement('div');
            empty.className = 'notif-empty';
            empty.textContent = 'No private messages';
            channelList.appendChild(empty);
        } else {
            privateKeys.forEach(key => {
                const username = key.replace('PRIVATE_', '');
                const msg = lastMessageByChannel[key];
                renderChannelItem(username, 'PRIVATE', username, 'fa-user', msg);
            });
        }
...
function renderChannelItem(name, type, target, icon, lastMsg) {
    const key = type === 'GLOBAL' ? 'GLOBAL_ALL' : (type === 'TEAM' ? `TEAM_${target}` : `PRIVATE_${target}`);
    ...
    if (lastMsg) {
        const timeAgo = Math.floor((Date.now() - lastMsg.timestamp) / 60000);
        timeStr = timeAgo < 1 ? 'Now' : (timeAgo < 60 ? `${timeAgo}m` : `${Math.floor(timeAgo/60)}h`);
        previewText = `${lastMsg.senderDisplay}: ${lastMsg.text}`;
    }

    // Style classes for avatars based on type
    let avatarClass = 'private'; 
    if (type === 'GLOBAL') avatarClass = 'global';
    if (type === 'TEAM') avatarClass = 'team';
    ...
    div.innerHTML = `
        ...
        <div class="channel-info">
            ...
            <div class="channel-preview">${previewText}</div>
        </div>
        <div class="channel-meta">
            <span class="channel-time">${timeStr}</span>
            <span class="channel-unread-badge ${unread > 0 ? 'show' : ''}">${unread}</span>
        </div>
    `;
    
    div.addEventListener('click', () => openChannel(type, target, name));
...
function openChannel(type, target, name) {
    activeChatChannel = { type, target, name };
    
    // Setup Typing Listener
    setupTypingListener();

    // --- FIX START: Save "Read" State ---
    const key = getActiveChannelKey();
    
    // Mark this channel as read as of NOW
    localStorage.setItem(`lastRead_${key}`, Date.now());
    // --- FIX END ---

    // Clear unread for this channel locally
    if (unreadCounts[key]) {
        totalUnreadCount -= unreadCounts[key];
        delete unreadCounts[key];
        updateBadge();
    }
    
    // Force a re-process to immediately remove badges from the UI
    processAllMessages();
...
function renderActiveChat() {
    ...
    const relevantMessages = allMessages.filter(msg => {
        if (activeChatChannel.type === 'GLOBAL') {
            return msg.channelType === 'GLOBAL';
        }
        if (activeChatChannel.type === 'TEAM') {
            return msg.channelType === 'TEAM' && msg.target === activeChatChannel.target;
        }
        if (activeChatChannel.type === 'PRIVATE') {
            // Match messages where target is ME and sender is THEM, OR target is THEM and sender is ME
            return (msg.channelType === 'PRIVATE') && 
                   ((msg.target === currentUsername && msg.senderDisplay === activeChatChannel.target) || 
                    (msg.target === activeChatChannel.target && msg.senderDisplay === currentUsername));
        }
        return false;
    });
    ...
    relevantMessages.forEach(msg => {
        // 1. Check Date
        const msgDate = new Date(msg.timestamp).toDateString(); // e.g., "Mon Jan 05 2026"
        
        // 2. Insert Separator if date changed
        if (msgDate !== lastDateStr) {
            const separator = document.createElement('div');
            separator.className = 'chat-date-separator';
            separator.innerHTML = `<span>${formatChatDate(msg.timestamp)}</span>`;
...
function renderMessage(msg) {
    ...
    if (msg.replyTo) {
        quoteHtml = `
            <div class="quoted-message" onclick="scrollToMessage('${msg.replyTo.id}')">
                <span class="quoted-sender">${msg.replyTo.sender}</span>
                <span class="quoted-text">${msg.replyTo.text}</span>
            </div>
        `;
    }
    ...
    div.innerHTML = `
        ${quoteHtml}
        <div class="msg-meta">
            <span>${isMe ? 'Me' : msg.senderDisplay}</span>
            <span>${timeStr} ${msg.isEdited ? '<i class="fa-solid fa-pen" style="font-size:0.6em;" title="Edited"></i>' : ''}</span>
        </div>
        <div>${msg.text}</div>
    `;

    // Interaction Listeners (Context Menu, Swipe)
        // Update visual state if already selected (when re-rendering)
    if (selectedMessageIds.has(msg.id)) div.classList.add('selected');

    // 1. CLICK (Toggle Selection if Mode is Active)
    ...
    // 2. LONG PRESS LOGIC (Trigger Selection Mode)
    let pressTimer;
    const startPress = () => {
        if (!isChatSelectionMode) {
    ...
    div.addEventListener('touchmove', (e) => { 
        ...
        // If moving (swiping), cancel the long press
        if (Math.abs(diff) > 10) cancelPress();

        // Standard Swipe to Reply (Only if NOT in selection mode)
        if (!isChatSelectionMode) {
            if ((!isMe && diff > 0 && diff < 100) || (isMe && diff < 0 && diff > -100)) div.style.transform = `translateX(${diff}px)`;
        }
    }, {passive: true});
    ...
    div.addEventListener('touchend', (e) => { 
        cancelPress(); 
        // Swipe Action
        if (!isChatSelectionMode && touchCurrentX !== 0 && Math.abs(touchCurrentX - touchStartX) > 50) {
            startReply(msg);
        }
        div.style.transform = ''; touchStartX = 0; touchCurrentX = 0;
    });

    // 4. CONTEXT MENU (Disable in selection mode)
...
if(chatSendBtn) chatSendBtn.addEventListener('click', handleChatAction);
if(chatInput) chatInput.addEventListener('keypress', (e) => {
    if (e.key === 'Enter') handleChatAction();
});
...
async function handleChatAction() {
    ...
    if (editingMessageId) {
        try {
            await updateDoc(doc(db, 'communications', editingMessageId), { text: text, isEdited: true });
            cancelEdit();
        } catch (e) { showToast("Error", "Could not update message."); }
        return;
    }

    // B. SEND NEW MESSAGE
    // Must have an active channel selected
    if (!activeChatChannel) {
        showToast("Error", "Select a chat first.");
        return;
    }

    let type = activeChatChannel.type;
    let target = activeChatChannel.target;

    // Prepare Reply Data
    let replyData = null;
    if (replyingTo) {
        replyData = { id: replyingTo.id, sender: replyingTo.sender, text: replyingTo.text };
    }
    ...
    try {
        await addDoc(collection(db, 'communications'), {
            ...
            senderDisplay: currentUsername || 'Unknown',
            channelType: type,
            target: target,
            timestamp: Date.now(),
            replyTo: replyData
        ...
        if (currentUser.email === ADMIN_EMAIL) cleanUpOldMessages(); 
...
window.scrollToMessage = function(msgId) {
    ...
    if (el) {
        ...
        showToast("Info", "Original message not loaded.");
    }
};

function updateBadge() {
    notifBadge.textContent = totalUnreadCount;
...
function renderNotifications() {
    ...
    // 1. Get the time when "Clear All" was last clicked
    const clearedTimestamp = parseInt(localStorage.getItem('notifs_cleared_timestamp') || 0);

    // 2. Filter: Not me, Last 24h, Newer than clear time
    let recentMsgs = allMessages.filter(m => 
        m.senderEmail !== currentUser.email && 
        (Date.now() - m.timestamp < 86400000) && 
        m.timestamp > clearedTimestamp
    );

    // 3. SORT: Newest First
    recentMsgs.sort((a, b) => b.timestamp - a.timestamp);
    ...
    recentMsgs.forEach(note => {
        // Check if message is unread based on channel read time
        const channelKey = getChannelKey(note);
        const lastReadTime = parseInt(localStorage.getItem(`lastRead_${channelKey}`)) || 0;
        const isUnread = note.timestamp > lastReadTime;

        const item = document.createElement('div');
        // Only add 'unread' class (Blue style) if it is actually new
        item.className = `notif-item ${isUnread ? 'unread' : ''}`;
        
        item.innerHTML = `
            <div class="notif-sender">${note.senderDisplay}</div>
            <div class="notif-text">${note.text}</div>
            <div class="notif-time" style="font-size: 0.65rem; color: #555; margin-top: 2px;">
                ${new Date(note.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
            </div>
        `;

        item.addEventListener('click', () => {
            // 1. Turn Grey immediately (remove blue style and dot)
            item.classList.remove('unread');

            // 2. Close dropdown
            toggleNotifications();
    
            // 3. Open Channel logic
            let type = note.channelType;
            let target = type === 'PRIVATE' ? note.senderDisplay : note.target; 
            
            if (type === 'PRIVATE' && note.senderDisplay === currentUsername) {
                target = note.target; 
            }
            
            openChannel(type, target, type === 'GLOBAL' ? 'Global Broadcast' : target);
            toggleChat(true);
        });
...
function showContextMenu(x, y, msg, isMe) {
    ...
    // Edit is still restricted to "Me" (you can't edit someone else's text)
    contextEdit.style.display = isMe ? 'flex' : 'none';

    // Only show Delete if I am the sender
    contextDelete.style.display = isMe ? 'flex' : 'none';
}

contextReply.addEventListener('click', () => {
    if (isChatSelectionMode) exitSelectionMode();
    startReply(selectedMessageData);
    chatContextMenu.style.display = 'none';
});
...
contextEdit.addEventListener('click', () => {
    if (isChatSelectionMode) exitSelectionMode();
    startEdit(selectedMessageData);
    chatContextMenu.style.display = 'none';
});

contextDelete.addEventListener('click', async () => {
    if (confirm("Delete message?")) await deleteDoc(doc(db, 'communications', selectedMessageData.id));
    if (isChatSelectionMode) exitSelectionMode();
    chatContextMenu.style.display = 'none';
});

function startReply(msg) {
    replyingTo = { id: msg.id, sender: msg.senderDisplay, text: msg.text };
    cancelEdit();
    replyPreviewContainer.style.display = 'flex';
    replySenderName.textContent = `Replying to ${replyingTo.sender}`;
    replyTextPreview.textContent = replyingTo.text;
    chatInput.focus();
}
...
async function cleanUpOldMessages() {
    // Changed 24 to 36 (36 hours ago)
    const limitTime = Date.now() - (36 * 60 * 60 * 1000); 
    const q = query(collection(db, 'communications'), where('timestamp', '<', limitTime));
...
// Dummy function to prevent errors if not present
...
// 1. Send "I am typing" to Firestore (Throttled to once every 2s)
chatInput.addEventListener('input', () => {
    if (!activeChatChannel || !currentUsername) return;
    
    const now = Date.now();
    if (now - lastTypingSent > 2000) {
        lastTypingSent = now;
        const key = getActiveChannelKey();
        const typingRef = doc(db, 'typing_status', key);
        // Set my username to current timestamp
        setDoc(typingRef, { [currentUsername]: now }, { merge: true }).catch(e => console.log("Typing err", e));
    }
});
...
function setupTypingListener() {
    ...
    typingUnsubscribe = onSnapshot(doc(db, 'typing_status', key), (docSnap) => {
        if (docSnap.exists()) {
            const data = docSnap.data();
            const now = Date.now();
            const activeTypers = [];

            // Check all fields in the doc
            Object.entries(data).forEach(([username, timestamp]) => {
                // If timestamp is within last 3 seconds AND it's not me
                if (username !== currentUsername && (now - timestamp < 3000)) {
            ...
            if (activeTypers.length > 0) {
                ...
                } else if (activeTypers.length < 4) {
                    typingText.textContent = `${activeTypers.join(', ')} are typing...`;
                } else {
...
// ==========================================
// CHAT SELECTION & BATCH ACTIONS
// ==========================================

function enterSelectionMode() {
    isChatSelectionMode = true;
    // We do NOT show the horizontal menu immediately. 
    // updateSelectionMenu() will decide whether to show the Side Menu or Horizontal Bar.
}
...
function exitSelectionMode() {
    ...
    // Hide BOTH menus
    chatSelectionMenu.style.display = 'none';
    chatContextMenu.style.display = 'none';
...
function updateSelectionMenu() {
    ...
    if (count === 1) {
        // CASE 1: Single Selection -> Show Context Menu (Side Logic)
        chatSelectionMenu.style.display = 'none'; // Hide Horizontal Bar
        
        // Get the single selected message ID and Data
        const [id] = selectedMessageIds;
        const msg = allMessages.find(m => m.id === id);
        const el = document.getElementById(`msg-${id}`);
        
        if (msg && el) {
            // Update global selection data for the context menu to use
            selectedMessageData = msg;
            
            // Calculate position for Context Menu (Right side of message)
            const rect = el.getBoundingClientRect();
            const isMe = currentUser && msg.senderEmail === currentUser.email;
            
            // Show Context Menu (offset slightly to not cover the message immediately)
            showContextMenu(rect.right - 20, rect.top + 10, msg, isMe);
        }
        
    /* --- NEW CODE --- */
    } else if (count > 1) {
        // CASE 2: Multi Selection -> Show Horizontal Bar
        chatContextMenu.style.display = 'none'; // Hide Context Menu
        chatSelectionMenu.style.display = 'flex'; // Show Horizontal Bar
        
        selectionCountText.textContent = `${count} Selected`;

        // 1. CHECK: Are ALL selected messages sent by me?
        let allMine = true;
        selectedMessageIds.forEach(id => {
            const msg = allMessages.find(m => m.id === id);
            // If message exists AND sender is not me, flag it
            if (msg && msg.senderEmail !== currentUser.email) {
                allMine = false;
            }
        });

        // 2. TOGGLE VISIBILITY: Hide Delete Button if mixed or received messages exist
        if (deleteSelectionBtn) {
            deleteSelectionBtn.style.display = allMine ? 'flex' : 'none';
        }
    }
}
...
if(copySelectionBtn) copySelectionBtn.addEventListener('click', () => {
    // 1. Gather text from selected messages
    // Use allMessages source to respect order (Oldest to Newest)
    const sortedSelection = allMessages
        .filter(m => selectedMessageIds.has(m.id))
        .sort((a, b) => a.timestamp - b.timestamp) // Ensure chronological order
        .map(m => `[${m.senderDisplay}]: ${m.text}`);

    const fullText = sortedSelection.join('\n');
    navigator.clipboard.writeText(fullText).then(() => {
        showToast("Copied", `${sortedSelection.length} messages copied.`);
        exitSelectionMode();
    });
});

if(deleteSelectionBtn) deleteSelectionBtn.addEventListener('click', async () => {
    ...
    selectedMessageIds.forEach(id => {
        ...
        if (msg && msg.senderEmail === currentUser.email) {
            const ref = doc(db, 'communications', id);
            batch.delete(ref);
            count++;
        }
    });
    ...
    if (count === 0) {
        return showToast("Info", "No sent messages selected to delete.");
    }

    try {
        await batch.commit();
        showToast("Deleted", `${count} messages removed.`);
        exitSelectionMode();
    } catch(e) {
...
function checkDeleteButtonVisibility() {
    ...
    selectedMessageIds.forEach(id => {
        const msg = allMessages.find(m => m.id === id);
        // If message exists AND I am not the sender
        if (msg && msg.senderEmail !== currentUser.email) {
            hasReceivedMessage = true;
        }
    });

    // logic: If ANY received message is present, HIDE the button.
    if (hasReceivedMessage) {
        deleteSelectionBtn.style.display = 'none';
    } else {
        deleteSelectionBtn.style.display = 'flex'; 
    }
}
